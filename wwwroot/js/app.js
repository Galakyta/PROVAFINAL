...
async function criarDespesa(e) {
    ...
    const novaDespesa = {
        ...
        imposto: dImposto.value ? Number(dImposto.value) : 0
    };
    ...
    listarDespesas(); // se nn a tabela nn atualiza automaticamente
}

async function deletarDespesa(e) {
    e.preventDefault();
    const id = delDespId.value;
    if (!id) {
        alert("o id é obrigatorio para deletes");
        return;
    }
    const resp = await fetch(`/despesas/${id}`, {
        method: "DELETE"
    });
    if (resp.status === 204 || resp.status === 200) {
        alert("Despesa deletada com sucesso!");
   }
    //204 ta funfando e 200 tmbm
    listarDespesas();  
    e.target.reset();  
}

async function atualizarDespesa(e) {
    e.preventDefault();

    const id = uDespId.value;

    const atualizado = {
        nome: uDespNome.value,
        data: uDespData.value,
        categoria: uDespCategoria.value,
        valor: uDespValor.value ? Number(uDespValor.value) : 0,
        imposto: uDespImposto.value ? Number(uDespImposto.value) : 0
    };
    const resp = await fetch(`/despesas/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    if (resp.ok) alert("Despesaa atualizada com sucesso!");
    else alert("ID de despesa não encontrado!");
    ...
    const dados = await resp.json();
        tabela.innerHTML += `
            <tr>
                ...
                <td>${dados.lucroBrutoMaquinas}</td>
                <td>${dados.despesasBrutas}</td>
                <td>${dados.lucroFinal}</td>
                </tr>
        `;
}
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    listar();          
    listarDespesas();  // a tabalea das dispesas
    relatorio();       // pra ja carregar o relatorio quando loadar
});
